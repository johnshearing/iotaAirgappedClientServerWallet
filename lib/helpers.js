...
// Lock, read, increment and unlock the the Globally Sequential Unique Id.
helpers.nextId = function()
{
    // Define the path were the lock folder is to be created.
    var dataDir = path.join(__dirname, '/../data/dbHistory/');
    ...
    fs.mkdirSync(dataDir + 'lock');

    // Call function to read the template then run the callback function defined here.
    fs.readFile(dataDir + 'sguid' + '.json', 'utf8', function(err,jsonStr)
    {
      if(!err && jsonStr && jsonStr.length >0) // if temple was read successfully
      {
        console.log(jsonStr);

        nextIdObject = helpers.parseJsonToObject(str);
      }
      else // the gsuid.json file was not read successfully.
      {
        console.log('This is the error message: ', err);
      }
    }); // End of: call to fs.readFile(...  

  // Read the nextId field in gsuid file.

  // Increment the nextId field in the gsuid file.

  // Release the lock on the gsuid file.
};
